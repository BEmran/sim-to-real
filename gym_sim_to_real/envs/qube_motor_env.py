...
class QubeMotorEnv(gym.Env):
    ...
    **Definition:**
    ...
    - The goal is to keep the pendulum at upward position (angle ~= 0).
    ...
    **STATE:**
    ...
    The 0 angle of the second link corresponds to the pendulum pointing upward. 
    **ACTIONS:**
    ...
    **REFERENCE:**
    ...
    MOTOR_COFF = 0.042          #: [N-m/A] Motor Current-torque coffecient 
    MOTOR_RESISTANCE = 8.4      #: [Ohm] Motor Resistance
 
    MAX_VEL_1 = 10.0 * np.pi
    MAX_VEL_2 = 10.0 * np.pi

    AVAIL_VOLTAGE = [-1.0, 0.0, +1.0]
    ...
    def __init__(self):
        print("hello old")
        self.viewer = None
        high = np.array([1.0, 1.0, self.MAX_VEL_1, self.MAX_VEL_2])
    ...
    def _terminal(self):
        s = self.state
        return bool( s[0] > np.pi/2 or s[0] < -np.pi/2 or  
                     s[1] > np.pi/2 or s[1] < -np.pi/2)
    ...
    def _dsdt(self, s, u, t):
        ...
        T = Km * (u - Km * dphi) / Rm
        eq1 = + (Mp * l * r * sin(2 * the) * (Jp * cos(dthe) * dphi**2 + Mp * g * l))/2 - Jp * (Mp * l * r * sin(the) * dthe**2 + Jp * dphi * sin(2 * t) * dthe - T)
        eq2 = + ((Jp * sin(2 * the) * dphi**2)/2 + Mp * g * l * sin(the)) * (Jp * sin(the)**2 + Ja) - Mp * l * r * cos(the) * (Mp * l * r * sin(the) * dthe**2 + Jp * dphi * sin(2 * the) * dthe - T)    

        det = + Ja * Jp + Jp**2 * sin(dthe)**2 - (Mp * l * r * cos(the))**2
